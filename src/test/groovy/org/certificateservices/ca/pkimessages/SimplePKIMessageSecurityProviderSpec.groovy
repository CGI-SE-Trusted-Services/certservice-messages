...
class SimplePKIMessageSecurityProviderSpec extends Specification {
	...
	@Shared SimplePKIMessageSecurityProvider prov
	
	@BeforeClass
	def setupSpec(){
		Properties config = new Properties();
		config.setProperty(SimplePKIMessageSecurityProvider.SETTING_SIGNINGKEYSTORE_PATH, this.getClass().getResource("/dummykeystore.jks").getPath())
		config.setProperty(SimplePKIMessageSecurityProvider.SETTING_SIGNINGKEYSTORE_PASSWORD, "tGidBq0Eep")
		config.setProperty(SimplePKIMessageSecurityProvider.SETTING_SIGNINGKEYSTORE_ALIAS, "test")
		
		config.setProperty(SimplePKIMessageSecurityProvider.SETTING_TRUSTKEYSTORE_PATH, this.getClass().getResource("/testtruststore.jks").getPath())
		config.setProperty(SimplePKIMessageSecurityProvider.SETTING_TRUSTKEYSTORE_PASSWORD, "foo123")
		prov = new SimplePKIMessageSecurityProvider(config);
	}
