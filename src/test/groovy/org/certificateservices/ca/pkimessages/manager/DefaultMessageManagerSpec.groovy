...
import org.certificateservices.ca.pkimessages.DefaultPKIMessageParser
import org.certificateservices.ca.pkimessages.DefaultPKIMessageParserSpec
...
class DefaultMessageManagerSpec extends Specification{
	...
	def setupSpec(){
		...
		parser.init(new DummyPKIMessageSecurityProvider(), config);
	...
	@Test
	def "Test to 1000 concurrent request and verify all responses are ok"(){
		...
		for(int i=0;i<numberOfConcurrentRequests;i++){
			String requestId = PKIMessageGenerateUtils.generateRandomUUID();
