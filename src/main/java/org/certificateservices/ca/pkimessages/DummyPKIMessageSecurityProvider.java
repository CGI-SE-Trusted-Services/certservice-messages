 ...
 *
 */
public class DummyPKIMessageSecurityProvider implements
	...
	private KeyStore getDummyKeystore() throws PKIMessageException{
		if(dummyKS == null){
			try {
				dummyKS = KeyStore.getInstance("JKS");
				dummyKS.load(this.getClass().getResourceAsStream("/dummykeystore.jks"), "tGidBq0Eep".toCharArray());
			} catch (Exception e) {
				throw new PKIMessageException("Error loading dummy key store: " + e.getMessage(),e);
			}
	...
	public PrivateKey getSigningKey() throws PKIMessageException {
		...
		try {
			...
			throw new PKIMessageException("Error fetching dummy signing key: " + e.getMessage(),e);
		}
	}
	...
	public X509Certificate getSigningCertificate()
		...
		try {
			...
			throw new PKIMessageException("Error fetching dummy signing certificate: " + e.getMessage(),e);
		}
	}

	/**
	 * 
	 * @see org.certificateservices.ca.pkimessages.PKIMessageSecurityProvider#isValidAndAuthorized(X509Certificate)
	 */
