...
public class X509DataOnlyKeySelector extends KeySelector {
	...
	private static Logger log = Logger.getLogger(X509DataOnlyKeySelector.class);

	private PKIMessageSecurityProvider pkiMessageSecurityProvider;
	
	/**
	 * Default constructor.
	 * @param pkiMessageSecurityProvider the provider to use.
	 */
	public X509DataOnlyKeySelector(PKIMessageSecurityProvider pkiMessageSecurityProvider){
	...
	private String findOrganisation(XMLCryptoContext context) throws KeySelectorException {
		...
		NodeList pkiMessageChilds = ctx.getNode().getParentNode().getChildNodes();
		for(int i= 0; i < pkiMessageChilds.getLength(); i++){
			Node next = pkiMessageChilds.item(i);
			if(next.getNodeName().equals("organisation")){
