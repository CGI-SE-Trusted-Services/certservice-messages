...
import org.certificateservices.ca.pkimessages.constants.AvailableCredentialStatuses;
...
public class DefaultMessageManager implements MessageManager, MessageResponseCallback{
					...
					if(itcr.getStatus() == RequestStatus.SUCCESS){
						...
						if( itcr.getCredentials() != null && itcr.getCredentials().getCredential() != null){
							for(Credential c : itcr.getCredentials().getCredential()){
								...
								try {
									byte[] revokeMessage = parser.genChangeCredentialStatusRequest(PKIMessageGenerateUtils.generateRandomUUID(),destination, responseMessage.getOrganisation(), c.getIssuerId(), c.getSerialNumber(), AvailableCredentialStatuses.REVOKED, REVOKE_REASON_REASONINFORMATION_CESSATIONOFOPERATION);
	 ...
	 */
	private MessageHandler getMessageHandler(Properties config, PKIMessageParser parser) throws PKIMessageException, IllegalArgumentException, IOException{
		try{
			String classPath = config.getProperty(SETTING_MESSAGEHANDLER_CLASSPATH);
			if(classPath == null){
				throw new PKIMessageException("Error no message handler configured with setting: " + SETTING_MESSAGEHANDLER_CLASSPATH);
			}			
			...
			throw new PKIMessageException("Error creating Message Handler: " + e.getMessage(),e);
		}
	}
	
	
	private long getTimeOutInMillis(Properties config) throws PKIMessageException{
		String timeout = config.getProperty(SETTING_MESSAGE_TIMEOUT_MILLIS, DEFAULT_MESSAGE_TIMEOUT_MILLIS);
		try{
			return Long.parseLong(timeout);
		}catch(Exception e){
			throw new PKIMessageException("Invalid timout value in configuration, check setting: " + SETTING_MESSAGE_TIMEOUT_MILLIS);
		}
	}

	public Object getConnectionFactory() throws PKIMessageException,
			IOException {
